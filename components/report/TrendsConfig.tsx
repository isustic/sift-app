import { Calendar, TrendingUp, Layers, Calculator, Filter } from "lucide-react";
...
export function TrendsConfig({
  ...
  const dateColumns = columns.filter((c) => c.col_type === "TEXT"); // Assume TEXT dates for now
      ...
      <div className="space-y-2">
        ...
        <div className="p-3 rounded-lg bg-primary/5 border border-primary/20">
          <p className="text-[11px] text-muted-foreground mb-2">
            Select a date column and time interval to analyze trends over time.
          </p>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-[10px] font-medium text-muted-foreground uppercase tracking-wider">
            ...
            Optional Grouping
            <span className="text-[9px] text-muted-foreground/60 normal-case">(add dimensions to split trends)</span>
          </div>
          <Button
            size="sm"
            variant="ghost"
        ...
        <div className="space-y-1.5">
          {dimensions.length === 0 ? (
            <p className="text-xs text-muted-foreground/50 py-2 italic">No additional grouping</p>
          ) : (
            dimensions.map((d, i) => (
